...
| 1 | Environment, secrets, budget & synthetic inputs | `ENV1`–`ENV4`, `SEC1`–`SEC4`, `LEAD1`–`LEAD3` | ✅ | ⬜ Not started |
| 2 | Conversation design (persona, dialog policy, literals) | `CONV1`–`CONV6` | ✅ | ⬜ Not started |
...
- [ ] `ENV1`/`ENV2` — fresh-venv install clean; every import pinned with `==`; `REALTIME_MODEL` **locked = `"gpt-4o-realtime-preview"`** (`OQ-VOICE-1` resolved; still verified against the real install at `ENV2`).
- [ ] `ENV3`/`ENV4` — `make test` clean from a fresh checkout; `import app.*` side-effect-free from an empty dir (lazy singletons `None`, no `.env`, no call).
- [ ] `SEC1` — no secret / **card number** (16-digit PAN, not the bare CVV) in any tracked file; `.env` gitignored; `.env.example` placeholders only; **`Home_Assignment_email.md` (redacted) + `REFERENCE/` gitignored** (Red-Team 2026-06-23).
- [ ] `SEC2`–`SEC4` — budget ledger correct; hard cap + per-call ceiling enforced before any dial; live sub-caps wired.
- [ ] `LEAD1`–`LEAD3` — synthetic inputs validate on load; no input value hardcoded in code/prompts.
- [ ] `CON1` (allowlist load) — the consent allowlist source validates on load (analogous to `LEAD1`); a malformed/empty allowlist is a clean explicit error, not silent allow-none (Red-Team 2026-06-23, Finding 9).
- [ ] `LIVE0` (provisioning readiness) — **day-1, in parallel with coding:** Vapi account + outbound number provisioned (allow for A2P/identity verification lead time), Cal.com account + API key + event type configured, OpenAI Realtime access verified on the key, a public HTTPS webhook tunnel available. **This is the #1 schedule risk** (Red-Team 2026-06-23, Findings 2/3) — start it before any other Stage-1 work.
**Status:** 🔄 In progress (started 2026-06-23) · **Reviewer gate:** ✅ (constants + budget/consent contracts).
...
**Status:** ⬜ Not started · **Reviewer gate:** ✅ (literals + Policy 4/6).
...
- **Status:** **Stage 0 ✅ complete** — green-lit 2026-06-23 16:57. Repo `git init`-ed; secret pre-flight
  CLEAN (sensitive files gitignored; no secret in any would-be-tracked file). **Stage 1 in progress** under
  the autonomous ORCHESTRATION loop; `LIVE0` provisioning owned by Asaf in parallel.
...
- **Next action:** Stage 1 (env / secrets / budget / consent / synthetic inputs / import-safety) via a cold
  `general-purpose` executer; PM re-runs the QA IDs + the `/code-review` gate; auto-advance per the loop.
