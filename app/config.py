...
# ---------------------------------------------------------------------------
# Repo root — all paths are built relative to this (no hardcoded absolute paths)
# ---------------------------------------------------------------------------
...
# consumers resolve the path via value_prop_path() only.
# ---------------------------------------------------------------------------
...
def value_prop_path() -> Path:
    """Return the path to the value-prop markdown (lazy; no read here).
    ...
    consumer hardcodes the data filename. Reads nothing — just resolves.
    """
...
# ===========================================================================
# Named constants — the single source of truth; never inline these elsewhere.
# ===========================================================================
...
# from MAX_LIVE_CALLS so the demo path is unchanged:
...
# --- byte-exact graded literals (copy byte-for-byte) ---
DISCLOSURE_LINE: str = (
...
# byte-exact and never model-generated, and never contradicts a booking with
# "I'll follow up by email". FAILSAFE_HANGUP_LINE
# stays the offline safe-terminal literal and the decline pre-close.
END_CALL_MESSAGE: str = "Thanks so much for your time. Goodbye!"

# --- the agent's callable functions ---
# name == schema name == dispatch key
...
# NOT live agent tools (retired, each verified live):
#  - `end_call`: a custom function returning JSON never actually hung up the call
#    (the agent rambled past "goodbye"). Termination is now pinned to the platform's
#    native end-call + the byte-exact END_CALL_MESSAGE.
#  - `qualify`: added a ~2.5s mid-call round-trip for routing the model can do
#    inline; survives only as the internal oracle behind the rubric's tailoring check.
...
# Reads os.environ ONLY when called — never at import time.
# ===========================================================================
...
# are never called under a hallucinated id.
...
def get_lead_context() -> tuple[str, str | None]:
    ...
    Call at runtime only (reads env via get_setting) — never at import.
    """
...
def load_env(dotenv_path: str | Path | None = None) -> None:
    ...
    Deliberately NOT called at import time (importing app.config reads no
